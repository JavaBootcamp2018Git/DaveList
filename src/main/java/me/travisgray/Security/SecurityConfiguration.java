...
/**
 * Created by ${TravisGray} on 2/14/2018.
 */
...
//                .access("hasAuthority('ROLE_USER') or hasAuthority('ROLE_ADMIN')")
                .authorizeRequests()
                .antMatchers("/","/list","/h2-console/**").permitAll()
                .antMatchers("/update/**","/test").access("hasAuthority('USER')")
                .antMatchers("/delete/**").access("hasAuthority('ADMIN')")
                .antMatchers("/add").access("hasAuthority('Teacher')")
...
//    remove the comments in the additional code. This is also the method in which you can configure how users are granted access to the appliaction if their details are stored in a database.
    ...
    protected void configure(AuthenticationManagerBuilder auth) throws Exception{
        ...
        auth.inMemoryAuthentication().
                withUser("DaveWolf").password("beastmaster").authorities("USER").
                and().
                withUser("Travis").password("password").authorities("ADMIN").
        and().
                withUser("Dim").password("password").authorities("Teacher");
